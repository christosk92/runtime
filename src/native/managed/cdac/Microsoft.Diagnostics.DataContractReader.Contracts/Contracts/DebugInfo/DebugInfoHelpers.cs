...
internal static class DebugInfoHelpers
{
    ...
    internal static IReadOnlyList<AsyncSuspensionInfo> DoAsyncInfo(NativeReader nativeReader)
    {
        ...
        for (uint i = 0; i < numSuspensionPoints; i++)
        {
            ...
            // DiagnosticNativeOffset is a uint32 in the encoder; deltas are signed to allow
            // backwards motion between suspension points only conceptually, but the cumulative
            // running offset is always a non-negative native code offset.
